...
(define Pride-Rock (instantiate place 'Pride-Rock)) ;;;;; 1. Instantiate Place: Dormitory
(define Kirin (instantiate place 'Kirin)) ;;;;; 1. Instantiate Plce: Kirin
(define Elephant-Graveyard (instantiate locked-place 'Elephant-Graveyard))
(define Harmony-Harmony-Harmony (instantiate garage 'Harmony-Harmony-Haromony))

(can-go Pride-Rock 'west Soda) ;;;;; 1. Connecting Dormitory to Soda
(can-go Soda 'east Pride-Rock)
(can-go Soda 'north Kirin) ;;;;; 1. Connecting Kirin to Soda
(can-go Kirin 'south Soda)
(can-go Pride-Rock 'south Elephant-Graveyard)
(can-go Elephant-Graveyard 'north Pride-Rock)
(can-go Pride-Rock 'east Harmony-Harmony-Harmony)
(can-go Harmony-Harmony-Harmony 'west Pride-Rock)
...
(define Mufasa (instantiate person 'Mufusa Pride-Rock)) ;;;;; 1. I AM MUFASA
...
(ask preacher 'set-talk "Jesus is going to the gym tonight, yeah yeah yeah")

(define street-person (instantiate person 'harry Telegraph-Ave))
(ask street-person 'set-talk "Brotha, spare some change?")

(define sproul-hall-exit ;;;;; A3
   (let ((count 0))
     (lambda ()
       (if (>= count 3)
	   ""
...
(define yo (instantiate person 'yo sproul-plaza))
...
(define potstickers (instantiate thing 'potstickers)) ;;;;; 1. Putting potstickers in Kirin
(ask Kirin 'appear potstickers)

(define robot-unicorn (instantiate thing 'robot-unicorn))
(ask Pride-Rock 'appear robot-unicorn)
