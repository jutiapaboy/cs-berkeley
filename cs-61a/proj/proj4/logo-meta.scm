;; Amanda Wong	cs61a-oc	Person A
;; Annie To	cs61a-od	Person B

;;; CORRECTED APRIL 25, 2010 -- see logo-apply
...
(define (eval-line line-obj env)
 (cond ((ask line-obj 'empty?) '=NO-VALUE=)
       (else (let ((value (logo-eval line-obj env)))
...
;;; (other procedures must be modified, too)
...
(define (variable? exp)
  (if (not (null? exp))
      (eq? (first exp) ':)
      #f))
;; makes it possible to print/show/type []
...
(define (handle-infix value line-obj env)
  (if (ask line-obj 'empty?)
      ...
      (let ((infix (ask line-obj 'next)))
        (if (memq infix '(+ - * / = < >))
	    (let ((op (lookup-procedure (de-infix infix)))
		  (new-value (eval-prefix line-obj env)))
	      (handle-infix (logo-apply op (list value new-value))
			    line-obj
			    env))
	    (begin (ask line-obj 'put-back infix)
		   value)))))	
...
(define (eval-definition line-obj)
  (let ((proc-name (ask line-obj 'next)))
    (define (find-formals)
      (if (ask line-obj 'empty?)
	  '()
	  (let ((token (ask line-obj 'next)))
	    (if (eq? token 'STATIC)
		'()
		(append (list (variable-name token)) (find-formals))))))
    (define (make-static-frame frame)
      (if (ask line-obj 'empty?)
	  frame
	  (let ((static-var (variable-name (ask line-obj 'next)))
		(static-val (logo-eval line-obj the-global-environment)))
	    (add-binding-to-frame! static-var static-val frame)
	    (make-static-frame frame))))
    (let ((formals (find-formals))
	  (static (make-static-frame (make-frame '() '())))
	  (body (retrieve-procedure-body)))
      (set! the-procedures
	    (cons (list proc-name 'compound (length formals) (cons formals body)
			static			;; added for Problem 9
			#f)			;; added for Problem A8
		  the-procedures))
      (logo-print (list proc-name 'defined)))
      '=NO-VALUE=))

(define (retrieve-procedure-body)
  (prompt "-> ")
  (let ((BO '()))
    (let ((line (logo-read)))
      (if (equal? line '(end))
          '()
          (set! BO (cons line (retrieve-procedure-body)))))
    BO))
...
;	((eq? (caar exps) 'output)
;	 (cadar exps))
;	(else (let ((return (logo-eval (make-line-obj (car exps)) env)))
;		(if (eq? return '=NO-VALUE=)
;		    (begin return
;			   (eval-sequence (cdr exps) env))
;		    (error "You don't say what to do with " return))))))
...
;; also selectors created below

(define (eval-sequence exps env stepped?)
  (if (null? exps)
      '=NO-VALUE=
      (begin
	(if stepped?
	    (begin (logo-type (car exps))
		   (prompt ">>>")
		   (logo-read)))
	(let ((return (eval-line (make-line-obj (car exps)) env)))
	  (cond ((eq? return '=STOP=) '=NO-VALUE)
		((and (pair? return) (eq? (car return) '=OUTPUT=))
		 (cdr return))
		((not (eq? return '=NO-VALUE=))
		 (error "You don't say what to do with " return))
		(else (eval-sequence (cdr exps) env stepped?)))))))

(define (step wd)
  (let ((proc (lookup-procedure wd)))
    (if (not proc)
	(error "I don't know how to " wd)
	(begin (set-car! (step-flag proc) #t)
	       '=NO-VALUE=))))
...
(define (unstep wd)
  (let ((proc (lookup-procedure wd)))
    (if (not proc)
	(error "I don't know how to " wd)
	(begin (set-car! (step-flag proc) #f)
	       '=NO-VALUE=))))
...
(add-prim 'bl 1 bl)
(add-prim 'word -2 word)			;;; Problem 3
(add-prim 'sentence -2 se)			;;; Problem 3
(add-prim 'se -2 se)				;;; Problem 3
(add-prim 'list -2 list)			;;; Problem 3
(add-prim 'fput 2 cons)

(add-prim 'sum -2 (make-logo-arith +))		;;; Problem 3
(add-prim 'difference 2 (make-logo-arith -))
(add-prim '=unary-minus= 1 (make-logo-arith -))
(add-prim '- 1 (make-logo-arith -))
(add-prim 'product -2 (make-logo-arith *))	;;; Problem 3
...
(add-prim 'step 1 step)				;;; Problem A8
(add-prim 'unstep 1 unstep)			;;; Problem A8

(add-prim 'test '(1) test)			;;; Problem B8
(add-prim 'iftrue '(1) iftrue)			;;; Problem B8
(add-prim 'ift '(1) iftrue)			;;; Problem B8
(add-prim 'iffalse '(1) iffalse)		;;; Problem B8
(add-prim 'iff '(1) iffalse)			;;; Problem B8
      ...
      (cond ((self-evaluating? token) token)
	    ...
	    ((right-paren? token)
		 ...
		 (logo-apply
		  ...
		  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
		  (cond ((list? num)
			 (cons env (collect-n-args (car num) line-obj env)))
			(paren-flag (collect-n-args num line-obj env))
			(else (collect-n-args (abs num) line-obj env)) )))
		  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	       )) )))
  (eval-helper #f))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;			APRIL 25, 2010			      ;;;;;;;;;;
;;;;;;;;;;		 FOUND AND CORRECTED A MISTAKE                ;;;;;;;;;;
;;;;;;;;;;          ORIGINAL CODE ABOVE, CORRECT CODE BELOW           ;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	 (eval-sequence (procedure-body procedure)
			(extend-environment
...
(define (variable-exists? var env)
  (define (env-loop env)
    (define (scan vars vals)
      (cond ((null? vars)
             (env-loop (enclosing-environment env)))
            ...
            (else (scan (cdr vars) (cdr vals)))))
    (if (eq? env the-empty-environment)
        #f			;; changed from (error "Unbound variable " var)
        (let ((frame (first-frame env)))
          (scan (frame-variables frame)
                (frame-values frame)))))
  (env-loop env))
;; this is exact same code as lookup-variable-value, except with the marked line edited