...
(define-class (line-object line)
  (instance-vars (tokens-read '())
		 (tokens-remaining line))
  (method (empty?)
	  (null? tokens-remaining))
  (method (next)
	  (let ((next-token (car tokens-remaining)))
	    (set! tokens-read (cons next-token tokens-read))
	    	;; remembers how much of the line has been read
	    	;; but reverses the order or the tokens
	    (set! tokens-remaining (cdr tokens-remaining))
	    next-token))
  (method (put-back token)
	  (set! tokens-read (cdr tokens-read))
	  (set! tokens-remaining (cons token tokens-remaining))))
...
(define (logo-type val)
  (cond ((null? val) (display "") '=NO-VALUE=)
        ((atom? val) (if (not (variable? val))
			 (begin (display val)
				'=NO-VALUE=)
			 (begin (display "")	;; makes it possible to print []
				'=NO-VALUE=)))
        ((= (length val) 1) (cond ((pair? (car val))
                                  (display "[")
                                  (logo-type (car val))
                                  (display "]")
                                  (logo-type (cdr val))
                                  '=NO-VALUE=)
				  (else (display (car val))
					'=NO-VALUE=)))
        ((null? (cdr val)) (begin (display (car val))
				  (display " ")
				  '=NO-VALUE=))
        (else (if (pair? (car val))
                  (begin (display "[")
			 (logo-type (car val))
			 (display "] ")
			 (logo-type (cdr val)))
                  (begin (display (car val))
			 (display " ")
			 (logo-type (cdr val)))))))
...
;;;(logo-meta.scm is also affected)

(define (make env var val)
  (let ((frame (first-frame env)))
    (cond ((variable-exists? var env)
	   (set-variable-value! var val env))
	  (else (define-variable! var val the-global-environment))))
  '=NO-VALUE=)
...
(define (test env t/f)
  (if (or (eq? t/f 'TRUE)
	  (eq? t/f 'FALSE))
      (begin (set-variable-value! " TEST" t/f env)
	     (lookup-variable-value " TEST" env))
      (error "Argument to test should be true or false")))

(define (iftrue env l)
  (let ((flag (lookup-variable-value " TEST" env)))
    (cond ((eq? flag 'TRUE)(run env l))
	  ((eq? flag 'FALSE) '=NO-VALUE=)
	  (else (error "You need to do a test first")))))

(define (iffalse env l)
  (let ((flag (lookup-variable-value " TEST" env)))
    (cond ((eq? flag 'FALSE)(run env l))
	  ((eq? flag 'TRUE) '=NO-VALUE=)
	  (else (error "You need to do a test first")))))
